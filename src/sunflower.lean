...
section partition
  ...
  begin
    ...
    { intros f hf,
      ...
      refine ⟨_, _, _⟩,
      { rcases hf with ⟨hf1, hf2, hf3, hf4⟩,
        rw mem_partitions_on,
        refine ⟨_, _, _⟩,
        { intros i hi,
          refine ⟨_, _⟩,
    ...
    { simp only [mem_filter, exists_prop],
      ...
      { intro i,
        ...
        { dsimp only,
          exact hV', },
        { dsimp only,
          have := finset.sdiff_subset s V,
          specialize hf3 i,
          exact subset_trans hf3 this, }, },
      { intros i j hij,
        cases i,
        { cases j,
          { exfalso,
          ...
          { dsimp only,
            specialize hf3 j,
            have : disjoint V (s \ V) := finset.disjoint_sdiff,
            exact finset.disjoint_of_subset_right hf3 this, }, },
        { cases j,
          { dsimp only,
            specialize hf3 i,
            have : disjoint V (s \ V) := finset.disjoint_sdiff,
            rw disjoint.comm at this,
            exact finset.disjoint_of_subset_left hf3 this, },
          { dsimp only,
            have hij' : i ≠ j,
            { simpa only [ne.def, add_left_inj] using hij, },
            exact hf4 i j hij', }, }, }, },
